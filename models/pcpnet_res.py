...
class PCPNetRes(nn.Module):
    ...
    def forward(self, points):
        x, trans, trans2, pointfvals = self.feat(points)
        ...
        n_est = pca_normal_est(points)
        n_est = n_est + x
        n_est = torch.nn.functional.normalize(n_est, p=2, dim=0)
        return n_est, trans, trans2, pointfvals, x


def pca_normal_est(points):
    " compute SVD on points covariance matrix to get initial estimate of the normal vector"
    with torch.no_grad():
        batch_size, n_points, D = points.shape
        x = points[:, 0, :]
        y = points[:, 1, :]
        z = points[:, 2, :]
        cg_x, cg_y, cg_z = torch.mean(x), torch.mean(y), torch.mean(z)
        x, y, z = x - cg_x, y - cg_y, z - cg_z
        cg_points = torch.cat([x.unsqueeze(-1), y.unsqueeze(-1), z.unsqueeze(-1)], dim=2)
        n = []
        for i in range(batch_size):
            _, _, v = torch.svd(torch.mm(cg_points[i].T, cg_points[i]))
            n.append(v[:, 2])
        n_est = torch.stack(n)
    return n_est
