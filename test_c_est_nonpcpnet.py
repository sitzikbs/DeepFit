...
def parse_arguments():
    ...
    parser.add_argument('--points_per_patch', type=int, default=0, help='number of points per patch, if 0 the training number of points is used')
...
def test_n_est(opt):
    ...
    for model_name in opt.models:
        ...
        if not opt.points_per_patch == 0:
            trainopt.points_per_patch = opt.points_per_patch
        # get indices in targets and predictions corresponding to each output
        trainopt.outputs = [] # no gt data
            ...
            while batch_offset < n_est.shape[0] and shape_ind + 1 <= len(dataset.shape_names):
                ...
                if shape_patches_remaining <= batch_patches_remaining:
                    ...
                    if shape_ind < len(dataset.shape_names):
                        ...
                        curv_prop = torch.zeros([shape_patch_count, 2])
                        weights_prop = torch.zeros([shape_patch_count, trainopt.points_per_patch])
                        beta_prop = torch.zeros(
                            [shape_patch_count, int((trainopt.jet_order + 1) * (trainopt.jet_order + 2) / 2)])
                        trans_prop = torch.zeros([shape_patch_count, 9])
